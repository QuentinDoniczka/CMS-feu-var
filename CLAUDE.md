...
- **Isolation à l'exécution** : la disjonction des empreintes protège les **fichiers**, pas l'**exécution**. `docker-compose.yml` monte le thème et l'extension en bind-mount `rw` depuis l'arbre partagé, donc **le code non committé d'une chaîne s'exécute dans les tests de toutes les autres** ; la base est partagée elle aussi. Une base par chaîne ne refermerait pas le trou — ce n'est pas la donnée qui change sous les pieds d'une chaîne, c'est le code qui la produit. Donc : **une seule chaîne à la fois exerce la stack** (le parallélisme s'arrête à la porte du conteneur), une chaîne qui teste par HTTP attend que celles qui écrivent du code de production aient commité, et l'orchestrateur vérifie le déblocage par `git status --short wp-content/` plutôt que sur parole. Un rouge isolé se rejoue avant d'être cru ; **un vert non reproductible n'est pas un vert**. Ne jamais assouplir une assertion au motif que la stack est instable. Voir `docs/decisions/isolation-des-chaines-paralleles.md` (tranché au lot #19/#69/#70).
