...
Voici celles que les gabarits portent **effectivement**, relevées dans le HTML servi — ce ne sont pas
des propositions, c'est ce qui est en ligne :

| Page | Description servie |
|---|---|
| La démarche | `Comment ce site relaie les statuts d'accès aux massifs forestiers des Bouches-du-Rhône : sources, licences, limites et point d'accès public au format JSON.` |
| Accessibilité | `Démarche d'accessibilité suivie pour ce site et moyen de signaler un problème d'accès à l'information.` |
| Mentions légales | `Éditeur, directeur de la publication, contact, et licences des données relayées par ce site.` |

Aucune ne dépasse 160 caractères, aucune ne reprend une chaîne officielle, aucune n'annonce un statut.

**Celle de la page « Accessibilité » n'annonce aucun résultat**, et c'est délibéré : une première
rédaction promettait « les résultats des vérifications menées » alors qu'aucun audit n'a été mené. Une
description est une promesse faite au lecteur **avant** qu'il n'ouvre la page ; celle-ci ne promet que
ce que la page tient.

> **Incohérence assumée et nommée plutôt que laissée à la revue** : ces trois descriptions sont de la
> rédaction, et elles vivent pourtant dans un gabarit — ce que l'arbitrage A-1 interdit à la prose. Il
> n'y a pas d'autre endroit : une `<meta>` doit être émise dans le `<head>`, avant `wp_head()`, et la
> faire venir du contenu demanderait `functions.php`, hors empreinte. Elles partent donc **non
> ratifiées** au même titre que le reste de la copie (contrat #18, arbitrage **A-14**, question Q-2).
...
La marche à suivre complète, les relevés déjà pris et ce que chacun prouve sont dans
[`../preuves-a11y-et-perf.md`](../preuves-a11y-et-perf.md) ; les relevés bruts sont dans
[`../releves/`](../releves/) et les scripts qui les produisent dans [`../outils/`](../outils/).

**Résultat de ces deux commandes au 16 août 2026** : un seul `h1` par page et une hiérarchie qui
commence à `h2` sur les trois pages ; aucune sous-ressource externe, et **aucune origine tierce du
tout** — mesuré en interceptant toutes les requêtes du navigateur, avec et sans JavaScript.
