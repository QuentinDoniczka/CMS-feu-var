...
| **A-14** | Ordre des filtres `authenticate` | `Ecluse::barrer` **prio 1**, cœur 20, `AccesCompte` 30, `Ecluse::constater` 40, `MessageConnexion` 45, `Deuxfacteurs` 50 | Contractuel. Le refus en priorité 1 rejette une requête verrouillée **sans jamais vérifier le mot de passe** : aucun calcul de hachage offert à l'attaquant, aucun oracle. Le refus de compte suspendu n'agit **que si `$utilisateur instanceof WP_User`**, sinon la suspension deviendrait un oracle d'existence de compte |
