...
| **A-24** | **L'étape 2 du second facteur reconsulte le verrou** — ajouté le 16 août 2026, sur mandat du lot | `Deuxfacteurs::traiter()` appelle `Ecluse::attente( $compte->user_login )` **après** la résolution du compte et **avant** le compteur de tentatives, la vérification du code et `wp_set_auth_cookie()`. Le refus **ne consomme aucune tentative**, **n'alimente pas l'écluse** et **ne brûle pas le jeton**. `Ecluse::message()` passe de `private` à `public` pour que les deux chemins rendent **la même phrase** | **L'étape 2 ne traverse pas `authenticate`.** Elle est atteinte par `login_form_massifs_2fa`, hors de `wp_signon` : ni `Ecluse::barrer` (1) ni `Ecluse::reaffirmer` (100) ne s'y exécutent. Un jeton d'étape 2 obtenu **avant** la pose d'un verrou pouvait donc encore ouvrir une session pendant ce verrou, dans toute la fenêtre de 300 s du jeton. **Portée exacte de ce que ça ferme** : `Deuxfacteurs.php` est le **seul** appelant de `wp_set_auth_cookie()` de toute l'empreinte de #13 — vérifié par `grep` sur `includes/security/**` —, donc fermer ce chemin ferme **toute** la classe de contournement. Le refus n'est pas comptabilisé pour la même raison que `massifs_trop_de_tentatives` ne l'est jamais dans la chaîne `authenticate` : une requête verrouillée n'est pas un code faux, et la compter permettrait de maintenir un utilisateur légitime dehors indéfiniment. Le jeton n'est pas brûlé pour que l'utilisateur légitime puisse reprendre après expiration du verrou ; son TTL de 300 s borne la fenêtre. Message unique parce que deux formulations pour un seul mécanisme feraient croire à deux protections distinctes. **Vérifié dans la stack qui tourne, sur un compte jetable détruit ensuite** : jeton obtenu hors verrou, verrou posé, puis étape 2 avec un **bon** code TOTP → refusée avec « Trop de tentatives de connexion. Réessayez dans 15 minutes. », **`set_auth_cookie` jamais déclenché**, `user_id = 0`, **essais restés à 0**, verrou survivant (890 s) ; puis verrou levé → l'étape 2 aboutit et pose le cookie. **LIMITE ASSUMÉE, ÉCRITE PLUTÔT QUE LAISSÉE CROIRE FERMÉE** : les clés de l'écluse sont l'IP hachée et le couple (identifiant × IP hachée), **jamais l'identifiant seul** (A-13). Un porteur de jeton qui **change d'origine** passe donc la vérification. C'est une conséquence **délibérée** d'A-13 — verrouiller sur le seul identifiant permettrait à n'importe quel visiteur d'éteindre le compte de démonstration, dont les identifiants sont publiés (§6). Ce cas n'est **pas** fermé, et ne doit pas l'être à ce prix |
